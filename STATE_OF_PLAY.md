...
3. **`58ed050`** — `fix(chat): three quality fixes for project chat output panel`. (1) Fresh just-generated outputs auto-expand instead of starting collapsed; older history still loads collapsed per design. The `handleSend` flow now adds the newest assistant message id to `expandedMessageIds` after `fetchHistory()` returns the enriched list. (2) Email-shape detection broadened: backend `CHAT_ADDENDUM` now instructs Ava to start email-shaped replies with `Subject:` (forward fix); `detectAvaOutputType` adds a greeting+sign-off fallback (`Hi/Dear/Hello/Hey` at start AND `Best/Regards/Cheers/Yours/Sincerely` near end, sign-off matched against last 400 chars only) so historical emails saved before the addendum still classify correctly. ActionBar's existing `case 'email'` branch renders Outlook/Gmail buttons unchanged. (3) Removed the unconditional nudge ("I can turn this into a deck or send it as an email — pick an option below") that rendered on every output type including `general` where the buttons it promised didn't exist. Single commit, 4 files: `chat/service.py`, `outputParser.ts`, `ActionBar.tsx`, `projects/[id]/chat/page.tsx`. No endpoint, schema, or migration changes.
4. **`28a7d99`** — `fix(layout): restore Calendar and People links to sidebar nav`. Both were removed in commit `924945f` (2026-04-27 "simplified sidebar" sweep), but on review the removal wasn't intended for either — both routes still existed and were reachable only by URL. Single-file frontend fix to `Sidebar.tsx`: adds `CalendarDays`/`Users` to lucide imports; appends `/people` to `PRIMARY_NAV` (with original `tourId='tour-people'`); inserts `/calendar` to `SECONDARY_NAV` between Inbox and Documents. No backend or routing change.
5. **`4e1c372`** — `feat(tasks): migrate TaskDetailModal to ui/Modal primitive`. Final of three bespoke-modal migrations; closes the tech-debt entry entirely. The largest of the trio (1,460 → 1,469 lines, 7 caller sites). Editable title preserved via the widened title prop (commit `0027c64`) — `titleNode` is the `editingTitle ? <input+save+cancel> : <span onClick={...}>{task.title}<span>✎</span></span>` swap, using span (not nested h2) under ui/Modal's outer h2. `handleCloseWithPrompt` (the unsaved-changes confirm dialog) is wired to ui/Modal's `onClose`, so Escape, backdrop click, AND the X button all route through the dirty-check — preserved bespoke UX. View-state machine (7 states: `idle`/`complete_*`/`delegate_*`) preserved verbatim inside the body. Chips row (priority, due, type, recurrence, status) moved from bespoke header into body's first row. UsageNudge inline banner stays where it was inside the body. Modal toast (position:fixed, bottom-centre) hoisted to fragment-sibling of `<Modal>` so it floats independently of the modal overlay. `maxWidth="max-w-xl"` (~576px, matches bespoke 580px). All 7 caller sites kept their conditional-mount pattern (`{state && <Modal task={state} ... />}`) — Conservative path: `open={true}` always passed internally; trade-off is no focus-return-on-close for this modal (bespoke had none either, so no regression — just no new gain). All other a11y wins (focus trap, ARIA, Escape, backdrop) apply.
